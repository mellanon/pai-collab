...
- Documented label taxonomy in CLAUDE.md: scope labels (`project/*`, `governance`), type labels, priority labels, cross-cutting labels (`seeking-contributors`, `security`, `trust`, `upstream-contribution`)
- Created `seeking-contributors` label for issues that are accessible entry points without requiring deep context
...
- The security review issue (#24) is the first `seeking-contributors` — scoped, self-contained, and explicitly inviting outside perspectives on a framework built by one operator
